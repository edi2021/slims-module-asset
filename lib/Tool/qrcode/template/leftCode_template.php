...
// set path
$assetPath = AWB . 'modules/asset/lib/Tool/' . pluginDirName();
        ...
        <div class="w-full bg-gray-100 h-screen">
            ...
            foreach ($chunked_barcode_arrays as $barcode_array):
                ...
                foreach ($barcode_array as $barcode):
                    // slicing number
                    $callNumber = ($barcode['itemcode'] . ' <br> ' . $barcode['name'] . ' <br> ' . $barcode['typename'] . ' <br> ' . $barcode['sourceitem']);
                    // shorting and slice it
                    $titleSlice = substr($barcode['name'], 0,5);
                    // get color
                    $color = 0;
                    // set template
                    // Barcode
                    ...
                    elseif ($globalSettings['codeType'] === 'Qrcode'):
                        ...
                        echo <<<HTML
                            <div style="width:{$responsiveWidth1}em; height: {$settingsTemplate['heightBox']}em; border: 1px solid black; margin-left: 8px; margin-top: 10px">
                                <div class="inline-block" style="width: {$responsiveWidth2}em ;height: {$settingsTemplate['heightBox']}em; border-left: 1px solid black">
                                    <span class="w-full block text-center text-sm" style="border-bottom: 1px solid black; background-color:{$color}">{$sysconf['library_name']}</span>
                                    <span class="w-full block text-left ml-3 text-md mt-8 font-bold {$settingsTemplate['callNumberFontSize']}">{$callNumber}</span>
                                </div>
                                <div class="inline-block float-left mr-2" style="width: 140.1px;">
                                    <small class="pl-2 pt-1">{$titleSlice} ...</small>
                                    <img id="img{$rowId}" data-code="{$barcode['itemcode']}" class="inline-block qrcode" style="width: {$widthQrcode}em; height: {$heightQrcode}em; margin-top: {$marginTop}em; margin-left: {$marginLeft}em; position: absolute;"/>
