...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  $item_ids = '';
  foreach ($_SESSION['qrcodes'] as $id) {
    $item_ids .= '\''.$id.'\',';
  }
  // strip the last comma
  $item_ids = substr_replace($item_ids, '', -1);
  // send query to database
  $item_q = $dbs->query('SELECT ai.itemcode, a.name, at.name as typename, asr.name as sourceitem FROM asset_item AS ai
    INNER JOIN asset AS a ON a.id=ai.assetid
    INNER JOIN asset_type AS at ON at.id=a.typeid
    LEFT JOIN asset_source AS asr ON asr.id=ai.source
    WHERE ai.itemcode IN('.$item_ids.')');
  ...
  // write to file
  $print_file_name = 'label_qrcode_gen_print_result_'.strtolower(str_replace(' ', '_', $_SESSION['uname'])).'.html';
