...
class masterAuthorization
{
    ...
    {
        // Set Box
        $Box = new Box($_SERVER['PHP_SELF'] . '?view=masterAuthorization', 'GET');

        $Box
            ->setTitle('Jenis Penguasaan Asset')
            ->setActionButton([
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterAuthorization&addForm=true', 'label' => 'Tambah Data', 'class' => 'btn btn-primary'],
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterAuthorization', 'label' => 'Daftar Asset', 'class' => 'btn btn-default']
                ])
            ->make();
    }
    ...
    private static function editData($id)
    {
        ...
        $Process = $Builder->prepare('select * from asset_authorization where id = :id');
    ...
    private static function addData()
    {
        ...
        // set fields
        $generalFields = [
            ['addTextField' => ['textarea', 'name', 'Nama Penguasaan' . '*', setData('name', $Data), 'rows="1" class="form-control"', 'Isikan nama barang yang hendak di record']],
    ...
    public static function render($process = false)
    {
        ...
        // Make Grid
        $Grid
            ->setTableSpec('asset_authorization')
            ->setColumn('id, name as "Penguasaan", lastupdate')
            ->setCriteria('name LIKE "%{keyword}%"', true)
            ->mutation(0, 'callback{mutationMstAuthorization}')
            ->make();
