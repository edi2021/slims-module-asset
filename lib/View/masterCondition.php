...
class masterCondition
{
    ...
    {
        // Set Box
        $Box = new Box($_SERVER['PHP_SELF'] . '?view=masterCondition', 'GET');

        $Box
            ->setTitle('Kondisi Asset')
            ->setActionButton([
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterCondition&addForm=true', 'label' => 'Tambah Data', 'class' => 'btn btn-primary'],
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterCondition', 'label' => 'Daftar Asset', 'class' => 'btn btn-default']
                ])
            ->make();
    }
    ...
    private static function editData($id)
    {
        ...
        $Process = $Builder->prepare('select * from asset_condition where id = :id');
    ...
    private static function addData()
    {
        ...
        // set fields
        $generalFields = [
            ['addTextField' => ['textarea', 'name', 'Kondisi' . '*', setData('name', $Data), 'rows="1" class="form-control"', 'Isikan nama barang yang hendak di record']],
    ...
    public static function render($process = false)
    {
        ...
        // Make Grid
        $Grid
            ->setTableSpec('asset_condition')
            ->setColumn('id, name as "Kondisi", lastupdate')
            ->setCriteria('name LIKE "%{keyword}%"', true)
            ->mutation(0, 'callback{mutationMstCondition}')
            ->make();
