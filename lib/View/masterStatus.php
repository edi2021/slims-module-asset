...
class masterStatus
{
    ...
    {
        // Set Box
        $Box = new Box($_SERVER['PHP_SELF'] . '?view=masterStatus', 'GET');

        $Box
            ->setTitle('Status Asset')
            ->setActionButton([
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterStatus&addForm=true', 'label' => 'Tambah Data', 'class' => 'btn btn-primary'],
                    ['url' => $_SERVER['PHP_SELF'] . '?view=masterStatus', 'label' => 'Daftar Asset', 'class' => 'btn btn-default']
                ])
            ->make();
    }
    ...
    private static function editData($id)
    {
        ...
        $Process = $Builder->prepare('select * from asset_status where id = :id');
    ...
    private static function addData()
    {
        ...
        // set fields
        $generalFields = [
            ['addTextField' => ['textarea', 'name', 'Status' . '*', setData('name', $Data), 'rows="1" class="form-control"', 'Isikan nama barang yang hendak di record']],
    ...
    public static function render($process = false)
    {
        ...
        // Make Grid
        $Grid
            ->setTableSpec('asset_status')
            ->setColumn('id, name as "Status", lastupdate')
            ->setCriteria('name LIKE "%{keyword}%"', true)
            ->mutation(0, 'callback{mutationMstStatus}')
            ->make();
